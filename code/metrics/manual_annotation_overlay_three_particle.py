...
VIDEO_PATH = PROJECT_ROOT / "data" / "final_inputs_sr" / "three_particles_video_sr.mp4"
CSV_PATH   = PROJECT_ROOT / "outputs" / "tracking_on_final_sr_videos_1" / "three_particles_video_sr" / "part3" / "tracks_merged.csv"
OUTPUT_DIR = PROJECT_ROOT / "outputs" / "manual_validation_three_particles"
    ...
    if event == cv2.EVENT_LBUTTONDOWN:
        ...
        if _drag_start is not None:
            ...
            if (bx2 - bx1) >= 3 and (by2 - by1) >= 3:
                ...
                # Auto-advance: 0 -> 1 -> 2 -> 0 -> ... so the common case
                # (draw P0, P1, P2 in order) is a smooth three-stroke flow.
                _active_id = (_active_id + 1) % N_PARTICLES
            _drag_start = None
            _drag_current = None
...
def _redraw(base_frame, boxes_display, drag_start, drag_current,
    ...
    msg2 = ("DRAG=box  '0'/'1'/'2'=set ID  Rclick=undo  SPACE=next  "
...
def annotate_video(video_path: Path, stride: int, expected_n: int,
    ...
    win = "Manual annotation - three particle (assign P0 / P1 / P2)"
...
def _assign_identity_to_pipeline(centers: dict, tracks: dict):
    ...
    for fi in sorted(centers.keys()):
        ...
        if not tracker_positions:
            continue
        score = min(len(ann), len(tracker_positions))
        candidates.append((score, fi, ann, tracker_positions))

    if not candidates:
        return {}

    candidates.sort(key=lambda t: (-t[0], t[1]))
    _, fi, ann, tracker_positions = candidates[0]
    ...
    if mapping:
        print(f"Identity mapping (anchored at frame {fi}, "
              f"{len(mapping)} simultaneous detections): "
...
def write_overlay_plot(tracks: dict, centers: dict, id_map: dict,
    ...
    """
    Displacement-from-start vs. time plot, identity-aware.

      Line  - pipeline trajectory per particle_id:
              ||(x(t), y(t)) - (x0, y0)|| where (x0, y0) is the FIRST
              recorded pipeline position for that particle.
      Dots  - manual annotations: each annotated dot uses the SAME
              origin (x0, y0) as the pipeline particle it was mapped to,
              and is plotted in the same color as that particle's line.

    A vertical jump of a colored dot onto a different-colored line is
    a clear visual indicator of an identity swap.
    """
    ...
    ax.set_title("Particle displacement — Pipeline vs Human (three particles)")
...
def write_error_summary(matches: list, out_dir: Path):
    ...
    lines = ["Manual-annotation vs pipeline error summary (three particles)",
             "=" * 60, ""]
    if not matches:
...
def main():
    ...
    print("\n── Manual annotation tool (three-particle, identity-aware) ──")
    print(f"Video : {VIDEO_PATH.name}")
    print(f"CSV   : {CSV_PATH.name}")
    print(f"Stride: every {FRAME_STRIDE} frames, expecting {N_PARTICLES} particle(s)\n")
    print("Controls:  LEFT-DRAG=box  '0'/'1'/'2'=set identity  RIGHT-click=undo")
    print("           SPACE=next  S=skip  R=reset  Q=quit")
    print("Tip: identity auto-advances 0->1->2 after each box, so usually you")
    print("     can just draw P0, P1, P2 in order, then press SPACE.\n")
